...
import com.dpworld.copilotworld.actions.ClearChatWindowAction;
...
public class ChatToolWindowPanel extends SimpleToolWindowPanel {
  ...
  public ChatToolWindowPanel(
    ...
    imageFileAttachmentNotification = new ToolWindowFooterNotification(() ->
        project.putUserData(AvatarKeys.IMAGE_ATTACHMENT_FILE_PATH, ""));
    upgradePlanLink = new ActionLink("Upgrade your plan", event -> {
      BrowserUtil.browse("https://avatar.ee/#pricing");
    });
    ...
    messageBusConnection.subscribe(ProviderChangeNotifier.PROVIDER_CHANGE_TOPIC,
        (ProviderChangeNotifier) provider -> {
          if (provider == ServiceType.AVATAR) {
    ...
    messageBusConnection.subscribe(AvatarUserDetailsNotifier.getAVATAR_USER_DETAILS_TOPIC(),
        (AvatarUserDetailsNotifier) userDetails -> {
          if (userDetails != null) {
  ...
  public void clearNotifications(Project project) {
    ...
    project.putUserData(AvatarKeys.IMAGE_ATTACHMENT_FILE_PATH, "");
    project.putUserData(AvatarKeys.SELECTED_FILES, emptyList());
  }
