...
public class ConfigurationComponent {
  ...
  public ConfigurationComponent(Disposable parentDisposable, ConfigurationState configuration) {
    table = new JBTable(new DefaultTableModel(
        ...
        new String[]{
            AvatarBundle.get("configurationConfigurable.table.header.actionColumnLabel"),
            AvatarBundle.get("configurationConfigurable.table.header.promptColumnLabel")
        }));
    table.getColumnModel().getColumn(0).setPreferredWidth(60);
    table.getColumnModel().getColumn(1).setPreferredWidth(240);
    table.getEmptyText().setText(AvatarBundle.get("configurationConfigurable.table.emptyText"));
    var tablePanel = createTablePanel();
    tablePanel.setBorder(BorderFactory.createTitledBorder(
        AvatarBundle.get("configurationConfigurable.table.title")));
    ...
    checkForPluginUpdatesCheckBox = new JBCheckBox(
        AvatarBundle.get("configurationConfigurable.checkForPluginUpdates.label"),
        configuration.isCheckForPluginUpdates());
    checkForNewScreenshotsCheckBox = new JBCheckBox(
        AvatarBundle.get("configurationConfigurable.checkForNewScreenshots.label"),
        configuration.isCheckForNewScreenshots());
    openNewTabCheckBox = new JBCheckBox(
        AvatarBundle.get("configurationConfigurable.openNewTabCheckBox.label"),
        configuration.isCreateNewChatOnEachAction());
    methodNameGenerationCheckBox = new JBCheckBox(
        AvatarBundle.get("configurationConfigurable.enableMethodNameGeneration.label"),
        configuration.isMethodNameGenerationEnabled());
    autoFormattingCheckBox = new JBCheckBox(
        AvatarBundle.get("configurationConfigurable.autoFormatting.label"),
        configuration.isAutoFormattingEnabled());
    autocompletionPostProcessingCheckBox = new JBCheckBox(
        AvatarBundle.get("configurationConfigurable.autocompletionPostProcessing.label"),
    ...
    mainPanel = FormBuilder.createFormBuilder()
        ...
        .addComponent(new TitledSeparator(
            AvatarBundle.get("configurationConfigurable.section.assistant.title")))
  ...
  private void addAssistantFormLabeledComponent(
    ...
    formBuilder.addLabeledComponent(
        new JBLabel(AvatarBundle.get(labelKey))
            .withBorder(JBUI.Borders.emptyLeft(2)),
        UI.PanelFactory.panel(component)
            .resizeX(false)
            .withComment(AvatarBundle.get(commentKey))
          ...
          try {
            ...
            if (value > 1.0 || value < 0.0) {
              return new ValidationInfo(
                  AvatarBundle.get("validation.error.mustBeBetweenZeroAndOne"),
                  component);
            }
          } catch (NumberFormatException e) {
            return new ValidationInfo(
                AvatarBundle.get("validation.error.mustBeNumber"),
                component);
          }
  ...
  class RevertToDefaultsActionButton extends AnActionButton {
    ...
    RevertToDefaultsActionButton() {
      super(
          AvatarBundle.get("configurationConfigurable.table.action.revertToDefaults.text"),
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
      var model = getModel();
      model.setRowCount(0);
      Arrays.stream(EditorActionsUtil.DEFAULT_ACTIONS_ARRAY).forEach(model::addRow);
  ...
  class KeymapActionButton extends AnActionButton {
    ...
    KeymapActionButton() {
      super(
          AvatarBundle.get("configurationConfigurable.table.action.addKeymap.text"),
          Nodes.KeymapEditor);
    }

    @Override
    public void actionPerformed(@NotNull AnActionEvent e) {
      var actionId = "avatar.AskChatgpt";
